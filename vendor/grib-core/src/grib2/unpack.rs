...
pub fn unpack_message(msg: &Grib2Message) -> crate::Result<Vec<f64>> {
    ...
    if matches!(dr.template, 2 | 3) {
        let expected = expected_coded_value_count(msg, msg.grid.nx as usize, msg.grid.ny as usize)
...
fn unpack_complex_spatial_scan_normalized_row_window(
    ...
    let dr = &msg.data_rep;
    let expected = expected_coded_value_count(msg, nx, ny)?;

    // A zero-group complex field is the GRIB2 representation of a constant
    // field.  There are no coded values to consume even though Section 5
    // still declares the logical point count.
    if dr.num_groups == 0 {
        return fill_scan_normalized_row_window_from_dense_values(
            msg,
            nx,
    ...
    if dr.spatial_diff_order == 0 {
        let mut groups = ComplexGroups::new(&msg.raw_data, dr, Some(expected))?;
        return fill_scan_normalized_row_window_from_dense_values(
            msg,
            nx,
            ny,
            ...
            || {
                groups
                    .next_packed()?
                    .map(|packed| packed_value_to_f64(packed, dr))
                    .transpose()
            },
        );
    }

    let mut decoder = SpatialDifferencingDecoder::new(&msg.raw_data, dr, Some(expected))?;
    fill_scan_normalized_row_window_from_dense_values(msg, nx, ny, y_start, y_end, || {
        decoder.next_scaled()
    })
}
...
fn packed_value_to_f64(packed: PackedComplexValue, dr: &DataRepresentation) -> Result<f64, String> {
    match packed {
        ...
        // Explicit complex-packing missing codes have the same canonical
        // meaning as bitmap-missing cells.  Preserve the raw substitutes in
        // DataRepresentation for provenance, but never admit their often
        // enormous sentinels into meteorological arithmetic or RWS storage.
        PackedComplexValue::PrimaryMissing | PackedComplexValue::SecondaryMissing => Ok(f64::NAN),
    }
}

fn expected_coded_value_count(msg: &Grib2Message, nx: usize, ny: usize) -> Result<usize, String> {
    let grid_points = nx
        .checked_mul(ny)
    ...
    let active_points = msg
        ...
        .unwrap_or(grid_points);
    let declared = msg.data_rep.section5_num_data_points as usize;
    if declared != 0 && declared != active_points {
        return Err(format!(
...
mod tests {
    ...
    fn test_complex_group_count_must_match_section5_points() {
        ...
        assert!(error.contains("describe 3 values, expected 4"), "{error}");
    }
    ...
    fn test_row_window_complex_missing_matches_full_decode() {
        ...
        let msg = Grib2Message {
            discipline: 0,
            reference_time: chrono::NaiveDate::from_ymd_opt(2026, 8, 14)
                .unwrap()
                .and_hms_opt(0, 0, 0)
                .unwrap(),
            grid: GridDefinition {
            ...
            product: ProductDefinition::default(),
            data_rep: spatial_missing_dr(2, 6),
            bitmap: None,
            raw_data,
        };
        let full = unpack_message_normalized(&msg).unwrap();
