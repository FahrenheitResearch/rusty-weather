...
impl Group {
    ...
    fn resolve_dense_links(
        ...
        for record in &records {
            ...
            let link_bytes = dense_link_payload(&managed_bytes);
            let mut link_cursor = Cursor::new(link_bytes);
...
fn dense_link_payload(bytes: &[u8]) -> &[u8] {
    if bytes.first().copied() != Some(1) && bytes.get(4).copied() == Some(1) {
        &bytes[4..]
    } else {
        bytes
    }
