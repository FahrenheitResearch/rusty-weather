    ...
    pub fn read_f64(&self, name: &str) -> WrfResult<Vec<f64>> {
        ...
        let out = match info.dtype {
            ...
            DType::I32 => raw
                ...
                .collect(),
            DType::U8 | DType::FixedString(_) => raw.iter().map(|&b| b as f64).collect(),
        };
        Ok(out)
    }
...
fn parse_dataspace(data: &[u8]) -> WrfResult<Vec<usize>> {
    ...
    let dim_start = match version {
        ...
        2 => 4usize,
        _ => {
            return Err(hdf5_err(format!("Unsupported dataspace version {version}")));
        }
    };
