...
mod tests {
    ...
    fn test_supercell_composite_enhanced() {
        ...
        assert!(
            (scp[0] - 11.25).abs() < 0.1,
            "SCP[0] expected ~11.25, got {}",
            scp[0]
        );
        assert!(
            (scp[1] - 0.075).abs() < 0.01,
            "SCP[1] expected ~0.075, got {}",
            scp[1]
        );
    }
    ...
    fn test_hot_dry_windy() {
        ...
        assert!(
            hdw > 100.0,
            "HDW should be large for hot/dry/windy: {}",
            hdw
        );
    }
    ...
    fn test_dendritic_growth_zone() {
        ...
        assert!(
            p_top < p_bottom,
            "p_top ({}) should be less than p_bottom ({})",
            p_top,
            p_bottom
        );
        assert!(
            (p_bottom - 745.0).abs() < 5.0,
            "p_bottom expected ~745, got {}",
            p_bottom
        );
    }
    ...
    fn test_dendritic_growth_zone_not_present() {
        ...
        let (p_top, p_bottom) = dendritic_growth_zone(&t, &p);
        assert!(
            p_top.is_nan() && p_bottom.is_nan(),
            "Should be NAN when DGZ not present"
        );
    }
    ...
    fn test_warm_nose_absent() {
        ...
        assert!(
            !warm_nose_check(&t, &p),
            "Should not detect warm nose in monotonic cooling"
        );
    }
    ...
    fn test_freezing_rain_composite_warm_surface() {
        ...
        assert!(
            frc.abs() < 0.001,
            "FRC should be 0 with warm surface: {}",
            frc
        );
    }
