...
use rustwx_sounding::{
    ExternalEcapeSummary, ExternalEcapeValue, ParcelFlavor, SoundingColumn, SoundingMetadata,
...
fn main() {
    ...
    let ecape = ExternalEcapeSummary {
        source: Some("rustwx-calc / metrust".to_string()),
        storm_motion: Some("Bunkers RM".to_string()),
        values: vec![
            ExternalEcapeValue {
        ...
        notes: vec![
            "Rendered as an external companion block. Native CAPE/CIN rows remain the sharprs parcel solution.".to_string(),
            "Use this as parcel-context annotation, not as evidence that sharprs computes entraining parcel physics.".to_string(),
        ],
    };

    let proof_dir = workspace_proof_dir();
    std::fs::create_dir_all(&proof_dir).expect("proof dir");
    let output = proof_dir.join("rustwx_sounding_demo_external_ecape.png");
    write_full_sounding_with_ecape_png(&column, &ecape, &output)
        .expect("render sounding with external ecape");
