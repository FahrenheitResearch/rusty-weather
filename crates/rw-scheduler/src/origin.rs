//! Declarative planning for a small public HTTPS origin catalog.
//!
//! This module deliberately does not download, publish, delete, or alias any
//! run. The current scheduler executor discovers one cycle per model, which is
//! not enough to maintain both a model's newest cycle and its newest complete
//! longest-horizon cycle. Operators and a future multi-cycle executor can use
//! this typed plan without pretending the existing executor provides that
//! behavior. The origin is a trusted conventional HTTPS server/signer, never a
//! relay; object-tier promotion and relay-mediated transport are outside this
//! scheduler policy.
...
use crate::plan::cycle_origin_unix;
...
/// Planning-only scheduler configuration for the public origin catalog.
///
...
impl OriginCatalogPlanConfig {
    /// Validate the audit gate and ensure every lane is backed by a configured,
    /// ready ingest capability. This still does not activate publication.
