...
- [x] Full gate + merge per finishing-a-development-branch (owner's call on merge timing vs GFS). **Gate DONE** (`cargo test --workspace` green; rw-glm fmt/clippy clean; branch pushed). **Merge deferred to the controller** per the Task 4 brief (do not self-merge).
